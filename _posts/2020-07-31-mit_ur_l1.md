...
\begin{enumerate}
\item No, we need the position and velocity of center of mass (or any other
well defined point on the robot body) in the state as well.
\item False, because no control input can produce a (large) vertical acceleration in the
COM.
\item False, because pure in-place rotation acceleration of the COM or acceleration the COM
